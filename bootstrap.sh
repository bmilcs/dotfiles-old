...
[[ -d /.zplugin ]] || mkdir -p ~/.zplugin && git clone https://github.com/zdharma/zplugin.git ~/.zplugin/bin
  ...
  # vim-plug
  [[ -d ${XDG_DATA_HOME:-$HOME/.local/share}/nvim/site/autoload/plug.vim ]] || echo "plug vim missing" && sh -c 'curl -fLo "${XDG_DATA_HOME:-$HOME/.local/share}"/nvim/site/autoload/plug.vim --create-dirs \
